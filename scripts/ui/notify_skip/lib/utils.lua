...
function render()
	...
	if state.is_idle and not Manager.disabled.idle_indicator then
		...
		ass:txt(center_x, center_y + icon_size / 1.2, 8, t('press Ctrl + Space for Quick Access Shortcuts'), {
			size = icon_size / 12, color = fg, opacity = config.opacity.idle_indicator,
		})
	end
